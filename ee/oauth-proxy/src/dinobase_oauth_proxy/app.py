...
The routes live on a standalone APIRouter so they can be:
  1. Run as a standalone proxy:  `dinobase-oauth-proxy`
  2. Mounted into a larger app:  `app.include_router(oauth_router, prefix="/oauth")`
"""
...
from dinobase_oauth_proxy.config import get_provider_credentials
...
# ---------------------------------------------------------------------------
# Standalone app — used by `dinobase-oauth-proxy` CLI
# ---------------------------------------------------------------------------

app = FastAPI(
    title="Dinobase OAuth Proxy",
    description="Handles OAuth authorization flows on behalf of the Dinobase CLI.",
    version="0.1.0",
)
