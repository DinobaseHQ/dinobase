...
- **`dinobase.mcp`** — sync wrappers around connected MCP servers. See the [Python API reference](../reference/python-api.md#mcp-client).
- **`DinobaseDB` / `QueryEngine`** — same interfaces the CLI and MCP server use internally. See the [Python API reference](../reference/python-api.md).
...
- [MCP Tools Reference → exec_code](../reference/mcp-tools.md#exec_code) — the raw tool schema and response format
- [Python API → MCP Client](../reference/python-api.md#mcp-client) — `call`, `tools`, `servers`, `search`, `instructions`
- [MCP Server Connectors](../connectors/mcp.md) — connecting MCP servers so their tools are callable from `exec_code`
- [Querying Data](querying.md) — when SQL is the better choice
