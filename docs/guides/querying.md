...
UPDATE and INSERT statements sent through `query` are intercepted and routed to the mutation engine. Instead of executing immediately, they return a preview with a `mutation_id`. See the [Mutations guide](mutations.md) for details.
