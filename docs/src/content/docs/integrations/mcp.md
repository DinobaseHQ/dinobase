...
See [Getting Started](/getting-started/) for a full walkthrough, [Connecting Sources](/guides/connecting-sources/) for the 100+ supported sources, and [Cloud Storage Backend](/guides/cloud-storage-backend/) for team/remote setups.
...
See [Syncing & Scheduling](/guides/syncing/) for more sync options.

## Generating configs with `mcp-config`

The `mcp-config` command generates the correct MCP configuration for any supported client:

```bash
dinobase mcp-config                # show configs for all clients
...
It auto-detects the `dinobase` binary path. If `dinobase` isn't on your PATH, it falls back to the full Python path.
...
- [Claude Code](/integrations/claude-code/) — CLI (recommended) or MCP
- [Claude Desktop](/integrations/claude-desktop/) — MCP via `claude_desktop_config.json`
- [Cursor](/integrations/cursor/) — MCP via `.cursor/mcp.json`
